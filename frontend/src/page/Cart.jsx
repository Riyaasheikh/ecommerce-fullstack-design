...
import { RiArrowLeftLine, RiShoppingBag2Fill, RiDeleteBin6Line } from "@remixicon/react";
import axios from "axios";

import Layout from "../layout/Layout";
import Featurebar from "../Components/ui/featurebar/Featurebar";
import SavedForLater from "../Components/ui/savedforlater/SavedForLater";
...
const Cart = () => {
    ...
    return (
      <Layout>
        <Container className="py-5 text-center">
          ...
          <Button as={Link} to="/product" variant="primary" className="mt-3 px-5 py-2">
            Explore Products
          </Button>
        </Container>
      </Layout>
  ...
  return (
    <Layout>
          ...
          <Col lg={8}>
            <Card className="border-0 shadow-sm overflow-hidden">
                    ...
                    <Row className="align-items-center">
                      <Col xs={3} md={2}>
                        <div className="bg-light rounded p-2 text-center">
                          <Image src={item.img} alt={item.title} fluid style={{ maxHeight: '80px' }} />
                        </div>
                      </Col>
                      <Col xs={9} md={5}>
                        <h6 className="fw-bold mb-1">{item.title}</h6>
                        <small className="text-muted d-block mb-2">Seller: Artel Market</small>
                        <Button 
                          variant="link" 
                      ...
                      <Col xs={6} md={3} className="mt-3 mt-md-0">
                        <div className="d-flex border rounded w-fit-content">
                          <Button variant="light" className="px-3 border-0 rounded-0" onClick={() => handleQtyChange(item.id, item.qty, -1)}>−</Button>
                          <div className="px-3 d-flex align-items-center fw-bold bg-white">{item.qty}</div>
                          <Button variant="light" className="px-3 border-0 rounded-0" onClick={() => handleQtyChange(item.id, item.qty, 1)}>+</Button>
                        </div>
                      </Col>
                      <Col xs={6} md={2} className="text-end mt-3 mt-md-0">
                        <span className="fw-bold h6">${(parsePrice(item.price) * item.qty).toFixed(2)}</span>
                      </Col>
                    </Row>
                  </div>
                ))}
              ...
              <Card.Footer className="bg-white p-3 border-top-0 d-flex justify-content-between">
                <Button as={Link} to="/product" variant="outline-primary" className="d-flex align-items-center gap-2">
                  <RiArrowLeftLine size={18} /> Continue Shopping
                </Button>
                <Button variant="link" className="text-danger text-decoration-none" onClick={clearCart}>
                  Clear All Items
                </Button>
              </Card.Footer>
            </Card>
          ...
          <Col lg={4}>
            <Card className="border-0 shadow-sm p-4 sticky-top" style={{ top: '20px' }}>
              ...
              <div className="d-flex justify-content-between mb-4">
                ...
                <span className="h5 fw-bold text-primary">${totals.total.toFixed(2)}</span>
              </div>
              <Button variant="success" className="w-100 py-3 fw-bold shadow-sm" onClick={handleCheckout}>
                Proceed to Checkout
              </Button>
            </Card>
          </Col>
      ...
      <SavedForLater />
      <div className="d-none d-lg-block"><Featurebar /></div>
      <div className="d-none d-lg-block"><Banner /></div>
    </Layout>
  );
};
